...
public class CSVWritter {
	...
	public void doWriteTicket(JSONArray ticketList, int ticketPageCounter, JSONArray customFieldsList, String zdDomain) {
		try {
			...
			if (snap.JAR_RUN) {
				...
				path = "C:\\Users\\Diastowo\\Documents\\Eclipse Output";
			}
			File file = new File(path + "\\JAVA_JENDEK\\" + zdDomain + "_ticketList_" + ticketPageCounter + ".csv");
			File cFile = new File(
					path + "\\JAVA_JENDEK\\" + zdDomain + "_commentsList_" + ticketPageCounter + ".csv");
...
//								System.out.println(ticketList.getJSONObject(i).names().get(j).toString() + " is NULL");
								ticketBw.write("\"" + ticketList.getJSONObject(i).names().get(j).toString() + " null\"|");
							}
						}
					}
				}
...
//							}
	...
	public void doWriteUsers(JSONArray usersList, int ticketPageCounter, String zdDomain) {
		try {
			...
			File file = new File(
					path + "\\JAVA_JENDEK\\" + zdDomain + "_UsersList_part_" + ticketPageCounter + ".csv");
	...
	public void doWriteTicketFields(kong.unirest.json.JSONObject ticketFields, String ticketFieldName, String zdDomain) {
	...
	public void doWriteTicketMetricsArrayList(ArrayList<JSONObject> ticketMetrics, int metricsPageCounter, String zdDomain) {
			...
			for (int i = 0; i < ticketMetrics.size(); i++) {
				if (i == 0) {
					for (int j = 0; j < ticketMetrics.get(i).names().length(); j++) {
						...
						} else if (ticketMetrics.get(i).names().get(j)
								.equals("first_resolution_time_in_minutes")) {
							bw.write(ticketMetrics.get(i).names().get(j).toString() + ".calendar| "
									+ ticketMetrics.get(i).names().get(j).toString() + ".business|");
						} else if (ticketMetrics.get(i).names().get(j)
	...
	private void doGenerateRowMetric(JSONArray ticketMetrics, int i, BufferedWriter bw) throws JSONException, IOException {
		for (int j = 0; j < ticketMetrics.getJSONObject(i).names().length(); j++) {
			if (ticketMetrics.getJSONObject(i).names().get(j).equals("reply_time_in_minutes")) {
	...
	private void doGenerateRowMetricArrayList(ArrayList<JSONObject> ticketMetrics, int i, BufferedWriter bw) throws JSONException, IOException {
		for (int j = 0; j < ticketMetrics.get(i).names().length(); j++) {
			if (ticketMetrics.get(i).names().get(j).equals("reply_time_in_minutes")) {
				...
				bw.write(
						ticketMetrics.get(i).getJSONObject("first_resolution_time_in_minutes").get("calendar")
								+ "|" + ticketMetrics.get(i).getJSONObject("first_resolution_time_in_minutes")
										.get("business")
								+ "|");
				...
				bw.write(ticketMetrics.get(i).getJSONObject("full_resolution_time_in_minutes").get("calendar")
						+ "|" + ticketMetrics.get(i).getJSONObject("full_resolution_time_in_minutes")
								.get("business")
						+ "|");
			} else if (ticketMetrics.get(i).names().get(j).equals("agent_wait_time_in_minutes")) {
				...
				try {
					bw.write(ticketMetrics.get(i)
							.get(ticketMetrics.get(i).names().get(j).toString()).toString() + "|");
				} catch (NullPointerException e) {
					bw.write("\"null\"|");
				}
			}
