				...
				if (snap.INC_COMMENTS) {
					if (!ticketList.getJSONObject(i).getString("status").equals("deleted")) {
						if (!(ticketList.getJSONObject(i).get("id").toString().equals("326")
							...
							doGetTicketCommments(ticketId, zdDomain);
							if (commentSuccess) {
...
//								ticketList.getJSONObject(i).put("user_involved", usersList);
						...
						System.out.println("Ticket ID: " + ticketList.getJSONObject(i).get("id") + " already deleted, skip comments");
					}
				}
				
				if (snap.INC_METRICS) {
					doGetTicketMetrics(ticketId, zdDomain);	
				}
			}
	...
	public void doGetTicketMetrics (String ticketId, String zdDomain) {
		System.out.print("Getting ticket metrics ID: " + ticketId + " - ");

		Unirest.get(snap.ZD_TICKET_METRICS_BYID_API(ticketId, zdDomain)).basicAuth(zD_Username, zD_Password).asJson()
	...
	public void doGetTicketCommments(String ticketId, String zdDomain) {
		...
		try {
			Unirest.get(snap.ZD_TICKET_COMMENT_API(ticketId, zdDomain)).basicAuth(zD_Username, zD_Password).asJson()
